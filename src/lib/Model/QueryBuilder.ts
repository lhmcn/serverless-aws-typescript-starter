...
import KeyExpression = DocumentClient.KeyExpression;
import ExpressionAttributeValueMap = DocumentClient.ExpressionAttributeValueMap;
import ProjectionExpression = DocumentClient.ProjectionExpression;
...
export default class QueryBuilder<T extends Model> {
	...
	sortKeyBetween(lowerBound: any, upperBound: any): this {
		if (this._sortKeyBetweenLowerBound !== lowerBound || this._sortKeyBetweenUpperBound !== upperBound) {
	 ...
	 */
	async query(deleteNoneDataProperties: boolean = true): Promise<QueryResult<T>> {
		...
		params.ExpressionAttributeValues = this.buildExpressionAttributeValues();
		...
		data.Items.forEach(dataItem => {
			...
			Model.fillItem(dataItem, returnItem, this._model, deleteNoneDataProperties, 2);
	...
	async all(deleteNoneDataProperties: boolean = true): Promise<T[]> {
		...
		const items: T[] = [];
		let pageKey: Key = undefined;

		// Loop until no pageKey returns
		do {
			const queryResult: QueryResult<T> = await this.query(deleteNoneDataProperties);
	 ...
	 */
	async firstOrDefault(defaultValue: T = null, deleteNoneDataProperties: boolean = true): Promise<T> {
		try {
	 ...
	 */
	private buildKeyConditionExpression(): KeyExpression {
		// Partition key value
		let n: number = this._attributeValues.push(this._partitionKeyValue)
		let expression: KeyExpression = `#partitionKey = :val${n}`;
	 ...
	 */
	private buildExpressionAttributeNames(): ExpressionAttributeNameMap {
		const names: ExpressionAttributeNameMap = {
	 ...
	 */
	private buildExpressionAttributeValues(): ExpressionAttributeValueMap {
		return this._attributeValues.reduce((obj: object, v: any, i) => ({
			...obj,
			[`:val${i + 1}`]: v
		}), {});
	}
	 ...
	 */
	private buildProjectionExpression(): ProjectionExpression {
		// Return all
