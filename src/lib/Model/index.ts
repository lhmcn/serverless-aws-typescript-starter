import QueryBuilder, {Key, QueryResult} from "./QueryBuilder";
...
export default abstract class Model {
	 ...
	 * @private
	 */
	private buildItem(): PutItemInputAttributeMap {
		// Check partition key
		if (!keyDataTypes[typeof (this[this.partitionKeyName])] || !this[this.partitionKeyName]) {
			throw "Partition key is required";
		}

		// Check sort key
		if (this.sortKeyName && (!keyDataTypes[typeof (this[this.sortKeyName])] || !this[this.sortKeyName])) {
		...
		// Update time if property names are not empty
		if (this.updatedAt) {
	 ...
	 */
	static async find<T extends Model>(partitionKeyValue: any, sortKeyValue: any = "", deleteNoneDataProperties: boolean = true): Promise<T> {

		// @ts-ignore
		const returnItem: T = new this();

		if (returnItem.indexName) {
			const result = await this.queryBuilder<T>().partitionKeyEquals(partitionKeyValue).sortKeyEquals(sortKeyValue).query(deleteNoneDataProperties);
			...
			if (data.Item) {
				...
				this.fillItem(data.Item, returnItem, returnItem, deleteNoneDataProperties, 2);
				return returnItem;
			}
		}
	 ...
	 */
	static async findOrDefault<T extends Model>(partitionKeyValue: any, sortKeyValue: any = "",
	                                            defaultValue: T = null, deleteNoneDataProperties: boolean = true): Promise<T> {
		try {
			return await this.find(partitionKeyValue, sortKeyValue, deleteNoneDataProperties);
		} catch (e) {
			if (e === "Item not found") {
				return defaultValue;
			} else {
	 ...
	 * @param sortKeyValue Value of the sort key
	 */
	static async delete<T extends Model>(partitionKeyValue: any, sortKeyValue: any = ""): Promise<void> {
		// @ts-ignore
	 ...
	 */
	static fillItem<T extends Model>(itemFrom: any, itemTo: any, itemTemplate: T, deleteNoneDataProperties: boolean = true, dynamoDBFlag: number = 0) {
		Object.getOwnPropertyNames(itemTemplate).forEach(k => {
			if (!noneDataProperties.has(k)) {
				if (dynamoDBFlag === 1) {
					...
					itemTo[k] = this.fromDynamoDBValue(itemFrom[k], itemTemplate[k]);
